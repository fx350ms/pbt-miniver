(function ($) {
    ...
    $.ajax({
        url: '/ConfigurationSetting/GetRMB', // Replace with the actual API endpoint
        type: 'GET',
        success: function (response) {
            if (response && response.result) {
                var rs = new Intl.NumberFormat('vi-VN', {
                    currency: 'VND',
                }).format(response.result);

                $('#rmbExchangeRate').text(rs );
            }
        },
        error: function () {
            $('#rmbExchangeRate').text('Error');
        }
    });
