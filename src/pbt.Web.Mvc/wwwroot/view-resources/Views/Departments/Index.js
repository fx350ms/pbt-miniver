(function ($) {
    ...
    var _$departmentsTable = _$table.DataTable({
        ...
        columnDefs: [
            ...
            {
                targets: 3, // Assuming the action column is at index 3
                data: null,
                sortable: false,
                width: 20,
                defaultContent: '',
                render: (data, type, row, meta) => {
                    return [
                        ...
                        `       <a class="dropdown-item edit-department" data-department-id="${row.id}" data-toggle="modal" data-target="#DepartmentEditModal">`,
                        `           <i class="fas fa-pencil-alt"></i> ${l('Edit')}`,
                        `       </a>`,
                        `       <a class="dropdown-item delete-department" data-department-id="${row.id}" data-department-name="${row.name}">`,
    ...
    $(document).on('click', '.edit-department', function (e) {
        ...
        e.preventDefault();
        abp.ajax({
            url: abp.appPath + 'Departments/EditModal?Id=' + departmentId,
            type: 'POST',
            dataType: 'html',
